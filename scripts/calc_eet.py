...
def run(workDir, config, visits, skipAg=False, skipDrp=False, useBackground=False, updateDB=False):
    ...
    if skipAg is False:
        cond.getConditionAg(visits=visits, showPlot=False, xaxis='taken_at', cc='flags', saveFig=True, figName='test_flags', dirName=figDir, corrColor=True, updateDB=updateDB)
    ...
    df_sky_background = qadb.query(f'SELECT * FROM sky WHERE {sqlWhere};')
    df_sky_noise = qadb.query(f'SELECT * FROM noise WHERE {sqlWhere};')
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--workDir",
        type=str,
        default='/work/kiyoyabe/erun/run22/get_condition',
